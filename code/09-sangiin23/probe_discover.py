#!/usr/bin/env python3
# Probe sangiin23 discoverability (summary + municipality tree)
...
def main() -> int:
    ...
    for url in (
        ...
        "https://www.soumu.go.jp/senkyo/senkyo_s/data/sangiin23/sangiin23_7.html",
        "https://www.soumu.go.jp/senkyo/senkyo_s/data/sangiin23/sangiin23_8.html",
        "https://www.soumu.go.jp/senkyo/senkyo_s/data/sangiin23/shikuchouson.html",
    ):
        try:
            r = get(session, url)
            print("OK", r.status_code, len(r.content), url)
        except Exception as exc:
            print("FAIL", type(exc).__name__, url.rsplit("/", 1)[-1])

    pages, links = discover(session, 23, chamber="sangiin")
